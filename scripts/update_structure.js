...
function generateFolderStructure() {
  ...
  const folders = fs
    ...
    .filter(dirent => dirent.isDirectory() && dirent.name !== 'testitem')
...
// 生成 TypeScript 文件
function generateTypeScriptFile(folderStructure) {
  const content = `// This file is auto-generated. Do not edit manually.
export const folderStructure = ${JSON.stringify(folderStructure, null, 2)} as const;
`;

  // 確保目錄存在
  const dir = path.dirname('src/utils/folderStructure.ts');
...
function main() {
  ...
  generateTypeScriptFile(folderStructure);
}
