...
const LoadingIndicator: React.FC = React.memo(() => (
  <li>
    ...
    >
      <svg className="mr-2 size-4 animate-spin" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const CurrentSelection: React.FC<{ folder: string; file: string }> = React.memo(({ folder, file }) => {
  ...
  return (
    <div className={className}>
      <p className="text-xs text-gray-500">
        <strong className="block font-medium text-gray-700 mb-1">Current Selection</strong>
...
const Footer: React.FC = React.memo(() => (
  <div className="sticky inset-x-0 bottom-0 border-t border-gray-100">
    <a href="#" className="flex items-center gap-2 bg-white p-4 hover:bg-gray-50 transition-colors duration-300" onClick={(e) => e.preventDefault()}>
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const folders = useMemo(() => Object.keys(folderStructure), [folderStructure]);
  const { isAnimating } = useContext(AnimationContext);
