...
export const useHeatmapData = (
  ...
  const loadData = useCallback(async (folder: string, file: string) => {
    ...
    try {
      ...
      setHeatmapData(null);
      const errorMessage = err instanceof Error ? err.message : ERROR_MESSAGES.LOAD_HEATMAP;
  ...
  useEffect(() => {
    if (isStructureLoaded && initialFolder && initialFile && 
      ...
      if (folderStructure[initialFolder] && 
        ...
        previousSelectionRef.current = { folder: initialFolder, file: initialFile };
        
        // Load data for new selection
        loadData(initialFolder, initialFile);
      }
    }
  }, [initialFolder, initialFile, isStructureLoaded, folderStructure, loadData]);
